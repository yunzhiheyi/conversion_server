...
export class EventsGateway {
  ...
  public async getHousingDetails(data: any, client: any) {
    ...
    var _this = this;
    var resData = []; // 结果
    $('.contents-listing-detail').each(function (_i, _elem) {
      var other_pic = []; // 房源图片
      var imgFloor = [];// 平面图片
      var videoUrl = []; // 视频地址
      var house_furniture = [];
      ...
      var house_vr = [];
      var details = []; // 详细信息
      var latitude = $(this).find('#map-accordion meta[itemprop="latitude"]').attr('content');
      var longitude = $(this).find('#map-accordion meta[itemprop="longitude"]').attr('content');
      // 图片资源
      $(this).find('.carousel-inner.infinite .item-img').each(function (i, elem) {
      ...
      var isRoom = $(this).find('[itemprop="numberOfRooms"]').text() === "Room";
      var resDataList: any = {
        _id: $(this).find('[data-listing-id]').first().attr('data-listing-id'),
        house_name: $(this).find('div.listing-title.text-transform-none').text(),
        price: $(this).find('.listing-detail-summary-bar-price .price').attr('content'),
        unit_price: $(this).find('.property-info-element.psf .price-value').text().replace(/\s*/g, "").replace(/\s*,\s*/, ''),
        ...
        house_bed: isRoom ? 1 : $(this).find('.property-info-element.beds').text().replace(/\s*/g, "").replace(/\n/g, ''),
        house_bath: $(this).find('.property-info-element.baths').text().replace(/\s*/g, "").replace(/\n/g, ''),
        land_size: _this.findDetailsInfo(details, 'Land Size'),
        loc_lng: longitude || 0,
        loc_lat: latitude || 0,
        bulid_year: $(this).find('.condo-profile-box__project-info .property-attr').first().find('.value-block').text().replace(/\s*/g, ""),
        house_vr: house_vr,
        floor_size: $(this).find('.property-info-element.area meta[itemprop="value"]').attr('content'),
        house_level: _this.findDetailsInfo(details, 'Floor Level'),
        add_detail: $(this).find('span[itemprop="streetAddress"]').text(),
        add_post: $(this).find('span[itemprop="postalCode"]').text(),
        ...
        house_describe_html: $(this).find('.listing-details-text').html().replace(/<h3[^>]*>[^<]*<\/h3>/gi, '').replace(/<a[^>]*>[^<]*<\/a>/gi, '').replace(/\n/g, '').replace(/ (^\s*)|(\s * $)/g, ""),
        house_describe: $(this).find('.listing-details-text').html().replace(/<h3[^>]*>[^<]*<\/h3>/gi, '').replace(/<br\/>/g, "\n").replace(/<\/?.+?\/?>/g, '').replace(/ (^\s*)|(\s * $)/g, ""),
        house_furniture: house_furniture,
        community_labels: community_labels,
        rent: _this.findDetailsInfo(details, 'Currently Tenanted') === 'NO' ? 17011002 : 17011001,
        community_id: $(this).find('#condo-widget_condo_url').attr('href') || ''
      }
      // 合租
      if (isRoom) {
        resDataList.house_classify = 'room';
      }
      resData.push(resDataList);
    });
    return resData
  }
  // 获取中介详情信息
  ...
  // 获取房源信息
  public async getHouseData(client: any, target_url: any, agentName: any) {
    let housingDetails = await this.getHousingDetails({
      target_url: target_url,
      stepsActive: 2,
      crack: '2',
    }, client);
    await Promise.all(housingDetails.map(async (item) => {
      // 关联房源信息爬取与录入
      var housingInfo = await this._housingModel.findOne({ _id: item._id });
      // 反向查询区域 
      var housingAddress = null;
      var housingAddInfo = null;
      if (item.loc_lng && item.loc_lat) {
        housingAddress = await this.getGeocode(item.loc_lat + ',' + item.loc_lng);
        housingAddInfo = this.toolsService.getIndexArray(this.areaScreen, housingAddress, 'enName')
      }
      var housingTypeInfo = this.toolsService.getIndexArray(this.toolsService.getTypeName(), item.type_three, 'enName');
      // 补充区域信息
      item.add_one = (housingAddInfo && housingAddInfo[0]) || { value: 14001, name: 'Singapore' };
      item.add_two = (housingAddInfo && housingAddInfo[1]) || { value: 14001006, name: 'N/A' };
      item.add_three = (housingAddInfo && housingAddInfo[2]) || { value: 14001006001, name: 'N/A' };
      // 楼层
      console.log('house_level1', item.house_level);
      item.house_level = this.toolsService.getLevel(item.house_level);

      console.log('house_level2', item.house_level);
      // 任期
      item.tenure = this.toolsService.getTenure(item.tenure);
      if (item.land_size === 'N/A' || !item.land_size) {
        item.land_size = 0.00
      }
      // 获取第三方视频连接缩略图
      if (item.video.length > 0) {
        ...
        await Promise.all(item.video.map(async (_itemData: any, _itemIndex: any) => {
          var _Thumbnail = '';
          if (_itemData && _itemData.url.indexOf('www.youtube.com') > -1) {
            ...
            _Thumbnail && (item.video[_itemIndex].thumbnail = _Thumbnail);
          }
          return _itemData;
        }));
      }
      // 补充类型
      item.type_one = housingTypeInfo[0] || { value: 16003, name: 'N/A' };
      item.type_two = housingTypeInfo[1] || { value: 16003001, name: 'N/A' };
      item.type_three = housingTypeInfo[2] || { value: 16003001001, name: 'N/A' };
      // sale
      if (!housingInfo) {
        await this._housingModel.create(item);
        client.emit('log', { event: 'log', type: 'success', data: '房源《' + item.house_name + '》入库成功...' });
      } else {
        await this._housingModel.updateMany({ _id: item._id }, { $set: { ...item, state: 2 } });
        client.emit('log', { event: 'log', type: 'success', data: '房源《' + item.house_name + '》数据已更新...' });
      }
      // 将房源所有的资源放一起利于下载
      await this._kPuppeteerService.download(item, 1);
      // 关联小区信息爬取与录入
      if (item.community_id) {
        this.logger.log('正在获取《' + item.house_name + '》关联小区信息...')
        client.emit('log', { event: 'log', type: 'loading', data: '正在获取《' + item.house_name + '》关联小区信息...' });
        var communityDetails: any = await this.getHouseCommunity({
          target_url: 'https://www.propertyguru.com.sg' + item.community_id,
          stepsActive: 3,
          crack: '2',
        }, client, item, housingInfo?.agent_id);
        // 获取第三方视频连接缩略图
        ...
        if (communityDetails && communityDetails.mangement_video && communityDetails.mangement_video.length > 0) {
          await Promise.all(communityDetails.mangement_video.map(async (_itemData: any, _itemIndex: any) => {
            ...
            if (_itemData && _itemData.url.indexOf('www.youtube.com') > -1) {
              ...
              _Thumbnail && (communityDetails.mangement_video[_itemIndex].thumbnail = _Thumbnail);
            }
            return _itemData;
          }));
        }
        var isCommunityData = await this._communityModel.findOne({ _id: item.community_id });
        if (!isCommunityData) {
          await this._communityModel.create(communityDetails);
          client.emit('log', { event: 'log', type: 'success', data: '房源《' + item.house_name + '》所在关联小区入库成功...' });
        } else {
          await this._communityModel.updateMany({ _id: item.community_id }, { $set: { ...communityDetails, state: 2 } });
          client.emit('log', { event: 'log', type: 'success', data: '房源《' + item.house_name + '》所在关联小区数据已更新...' });
        }
        // 将房源所有的资源放一起利于下载
        var communityInfo = await this._communityModel.findOne({ _id: item.community_id });
        if (communityInfo) {
          let complete = await this._kPuppeteerService.download(communityInfo, 2)
          if (complete) {
            this.logger.log('已完成《' + agentName + '》数据补充...')
            client.emit('log', { event: 'log', type: 'success', data: '已完成《' + agentName + '》数据补充...' });
          }
        }
      } else {
        this.logger.log('《' + item.house_name + '》没有关联小区信息...')
        client.emit('log', { event: 'log', type: 'warning', data: '《' + item.house_name + '》没有关联小区信息...' });
      }
    }));
    return new Promise((resolve, reject) => {
      resolve(true);
  ...
  async puppeteer(@MessageBody() data: any, @ConnectedSocket() client: WebSocket): Promise<any> {
    if (data.type === 'start') {
      ...
      return { event: 'events', data: resData }
    }
  }
